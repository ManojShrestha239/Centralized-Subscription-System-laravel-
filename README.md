...
Create a new file at `app/Http/Middleware/CheckSubscriptionMiddleware.php` and add:
...
class CheckSubscriptionMiddleware
{
    public function handle(Request $request, Closure $next)
    {
        ...
        $domain = "http://bihani.soa.local/";
        $apiSecret = "KhsezJqIVoneoNnbku9u3JFASxNLPkvR9Ap7dCA0";
        $url = "http://subscription.system.local:81/";
        ...
        $signature = hash_hmac('sha256', $timestamp . $domain . $nonce, $apiSecret);
        $baseUrl = $url;
        ...
        try {
            ...
            if (($body['status'] ?? '') === 'expired') {
                $redirectUrl = $body['redirect_url'] ?? $redirect_Url;
            ...
            return response()->json([
                'status' => 'error',
                'message' => 'Subscription API error'
            ], 500);
        }
    }
}
